...
class ManagerDataHonorInstrukturDetail extends CI_Controller {
...
//        $absen = $this->db->query("Select * from karyawan where id_karyawan = $id");
        foreach ($bulan->result() as $key) {
            $honor = $this->db->query("Select nama,jabatan,id_karyawan,id_absen,sum(gaji) as gaji,rekomendasikelas as level,tanggalmasuk from absensiswa join karyawan using(id_karyawan) join honor using(id_honor) join detailsiswa using(id_siswa) where id_karyawan = $key->id_karyawan and month(tanggalmasuk) = '$key->bulan' group by id_karyawan,month(tanggalmasuk)");
        ...
        $data['honor'] = $honor;
        $data['absen'] = $absen;
        $this->load->view('manager_data_honor_instruktur_detail.html', $data);
    }
