...
class ManagerDataAbsensiInstruktur extends CI_Controller {
    ...
    public function index() {
        ...
        $data['siswa'] = $siswa;
        $query = $this->db->query("Select id_karyawan,tanggalmasuk,nama,jammasuk,jampulang from absenkaryawan join karyawan using(id_karyawan) where jabatan like 'Instruktur%'")->result();
        $data['instruktur'] = $query;
        if ($name > 0) {
            $this->load->view('manager_data_kehadiran_instruktur.html', $data);
        } else {
            redirect('login');
        }
    }
